...
// Icon element types for multi-element icons
type IconElement =
  | { type: 'path'; d: string }
  | { type: 'circle'; cx: number; cy: number; r: number }

// Icon definition can be a simple path string or a complex definition
interface IconDefinition {
  elements: IconElement[]
  variant?: 'stroke' | 'fill'
}

type IconPathOrDefinition = string | IconDefinition

// Icon definitions (paths from lucide-static and custom icons)
const iconDefinitions: Record<IconName, IconPathOrDefinition> = {
  // Lucide stroke-based icons (simple path strings)
...
// Helper to check if definition is complex
function isComplexDefinition(def: IconPathOrDefinition): def is IconDefinition {
...
export function Icon({ name, size = 'md', class: className = '' }: IconProps) {
  ...
  // Handle complex definitions (multi-element or fill-based)
  if (isComplexDefinition(definition)) {
    const isFill = definition.variant === 'fill'
    ...
    return (
      <svg
        ...
        aria-hidden="true"
      >
        {renderElements(definition.elements)}
      </svg>
    )
  }

  // Handle simple path string (stroke-based)
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={pixelSize}
