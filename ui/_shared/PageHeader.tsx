...
export function PageHeader({ title, description, "data-key": __dataKey, __listIndex }: { title: string; description: string; "data-key"?: string | number; __listIndex?: number }) {
  ...
  try {
    ...
    __needsThis = !__outputScripts.has('PageHeader')
    if (__needsBarefoot) __outputScripts.add('__barefoot__')
    if (__needsThis) __outputScripts.add('PageHeader')
  ...
  const __thisSrc = (manifest as any)['PageHeader']?.clientJs
  ...
  try {
    ...
    if (__isRoot) {
      c.set('bfRootComponent', 'PageHeader')
    }
  } catch {
    ...
    <>
      ...
      {__isRoot && __hasHydrateProps && (
        <script
          ...
          data-bf-props="PageHeader"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(__hydrateProps) }}
        />
      )}
      <div {...(__dataKey !== undefined ? { "data-key": __dataKey } : {})} data-bf-scope="PageHeader" className="space-y-2"><h1 className="text-3xl font-bold tracking-tight text-zinc-50">{title}</h1><p className="text-zinc-400 text-lg">{description}</p></div>
    </>
  )
}
