...
export function Subsection({ title, children, "data-key": __dataKey, __listIndex }: { title: string; children: any; "data-key"?: string | number; __listIndex?: number }) {
  ...
  try {
    ...
    __needsThis = !__outputScripts.has('Subsection')
    if (__needsBarefoot) __outputScripts.add('__barefoot__')
    if (__needsThis) __outputScripts.add('Subsection')
  ...
  const __thisSrc = (manifest as any)['Subsection']?.clientJs
  ...
  try {
    ...
    if (__isRoot) {
      c.set('bfRootComponent', 'Subsection')
    }
  } catch {
    ...
    <>
      ...
      {__isRoot && __hasHydrateProps && (
        <script
          ...
          data-bf-props="Subsection"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(__hydrateProps) }}
        />
      )}
      <div {...(__dataKey !== undefined ? { "data-key": __dataKey } : {})} data-bf-scope="Subsection" data-bf="0" className="space-y-4"><h3 className="text-lg font-medium text-zinc-100">{title}</h3>{typeof children === 'function' ? children() : children}</div>
    </>
  )
}
