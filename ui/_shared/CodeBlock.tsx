...
export function CodeBlock({ code, lang, "data-key": __dataKey, __listIndex }: { code: string; lang?: string; "data-key"?: string | number; __listIndex?: number }) {
  ...
  try {
    ...
    __needsThis = !__outputScripts.has('CodeBlock')
    if (__needsBarefoot) __outputScripts.add('__barefoot__')
    if (__needsThis) __outputScripts.add('CodeBlock')
  ...
  const __thisSrc = (manifest as any)['CodeBlock']?.clientJs
  ...
  try {
    ...
    if (__isRoot) {
      c.set('bfRootComponent', 'CodeBlock')
    }
  } catch {
  ...
  if (typeof code !== 'function') __hydrateProps['code'] = code
  if (typeof lang !== 'function') __hydrateProps['lang'] = lang
    ...
    <>
      ...
      {__isRoot && __hasHydrateProps && (
        <script
          ...
          data-bf-props="CodeBlock"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(__hydrateProps) }}
        />
      )}
      <div {...(__dataKey !== undefined ? { "data-key": __dataKey } : {})} data-bf-scope="CodeBlock" className="relative"><pre className="p-4 bg-zinc-950 text-zinc-300 rounded-lg overflow-x-auto text-sm font-mono border border-zinc-800"><code>{code}</code></pre></div>
    </>
  )
}
