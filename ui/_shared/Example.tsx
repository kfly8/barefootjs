...
export function Example({ title, code, children, "data-key": __dataKey, __listIndex }: { title?: string; code: string; children: any; "data-key"?: string | number; __listIndex?: number }) {
  ...
  try {
    ...
    __needsThis = !__outputScripts.has('Example')
    if (__needsBarefoot) __outputScripts.add('__barefoot__')
    if (__needsThis) __outputScripts.add('Example')
  ...
  const __thisSrc = (manifest as any)['Example']?.clientJs
    ...
    <>
      ...
      {__isRoot && __hasHydrateProps && (
        <script
          ...
          data-bf-props="Example"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(__hydrateProps) }}
        />
      )}
      <div {...(__dataKey !== undefined ? { "data-key": __dataKey } : {})} data-bf-scope="Example" className="space-y-4">{title ? <h3 className="text-lg font-medium text-zinc-100">{title}</h3> : null}<Preview>{typeof children === 'function' ? children() : children}</Preview><CodeBlock code={code} /></div>
    </>
  )
}
